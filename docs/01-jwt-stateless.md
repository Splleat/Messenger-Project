...
그렇다고 JWT가 무조건 나쁜 선택이라고 말하고 싶은 것이 아니다. JWT는 실제로 많은 곳에서 채택되는 만큼 분명히 세션에 비해 강점이 존재할 것이다. 중요한 것은 "JWT는 현대적이고 확장 가능한 방식"이라는 고정 관념에서 벗어나, 실제 요구사항에 맞는 방식인지 따져볼 필요가 있다는 것이다.
