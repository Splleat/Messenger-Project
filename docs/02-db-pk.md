...
UUIDv7과 TSID는 뒷 부분에 붙는 랜덤/시퀀스 비트 덕분에 다음 ID를 예측하는 공격으로부터 안전하다. 하지만, 두 방식 모두 맨 앞자리에 타임스탬프가 위치하고 있어 ID 자체만으로 생성 시간을 정확히 역산할 수 있다는 특징이 있다.
...
새로운 기술을 도입하는 것도 마찬가지로 단순히 그 기술을 이해하고 도입하는 것만으로 끝나지 않는다. 이번 프로젝트에서 겪은 `SimpleJpaRepository`의 `save()` 문제와 JavaScript `number` 정밀도 문제를 통해 새로운 기술을 도입할 때는 기술 자체뿐 아니라 기존 생태계와의 상호작용까지 고려해야 한다는 점을 배울 수 있었다. 