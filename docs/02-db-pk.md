...
메시지 앱은 읽기(조회)보다 쓰기(생성) 비율이 압도적으로 높은 도메인 특성을 가지고 있다.

메신저 서버에서 메시지는 가장 빈번하게 생성되는 데이터이다. 사용자가 메시지를 보낼 때마다 INSERT 쿼리가 발생하고, 메시지 목록을 조회할 때마다 대량의 데이터를 읽어야 한다. 또한 메시지는 시간 순으로 정렬되어야 하고, 분산 환경에서도 고유해야 한다.

이러한 요구 사항을 정리하면 다음과 같다.
1. **쓰기 성능**: 빈번한 INSERT에서 인덱스 삽입 성능이 중요하다.
2. **시간순 정렬**: 메시지는 페이징을 위해서 생성 시간 순으로 정렬되어야 한다.
...
2. 36바이트(바이너리로 저장 시 16바이트)로 Auto Increment(8바이트)보다 인덱스 크기가 크다.
3. `5a9b8f2d-8c4e-4b2a-9e1f-3c5d7a9b8f2d`와 같은 형태라 가독성이 매우 떨어진다.

**UUIDv7**

UUID의 성능 문제를 개선하기 위해 등장한 버전이다. 앞부분에 타임스탬프를 포함하여 시간 순 정렬이 가능하기 때문에 UUIDv4에 비해 페이지 분할 문제를 크게 줄일 수 있다. 하지만 여전히 36바이트(바이너리로 저장 시 16바이트)로, 8바이트인 BIGINT에 비해 인덱스 크기가 최소 2배이다.

### 1.3. TSID 선택

PK 후보를 검토하던 도중 TSID(Time-Sorted Unique Identifier)에 대해 알게 되었다.

TSID는 8바이트 BIGINT와 완벽하게 호환되면서도 시간 순 정렬과 전역 고유성을 모두 만족한다.

|         | Auto Increment | UUIDv4 | UUID v7 | TSID   |
|---------|----------------|--------|---------|--------|
| 쓰기 성능   | 최상             | 나쁨     | 좋음      | 좋음     |
| 시간 순 정렬 | 가능             | 불가능    | 가능      | 가능     |
| 전역 고유성  | 조건부 가능 (단일 DB) | 가능     | 가능      | 가능     |
| 크기      | 8바이트           | 16바이트  | 16바이트   | 8바이트   |
| 보안 (예측 난이도) | 취약 | 우수 | 우수 | 비교적 우수 |

따라서 요구 사항을 모두 만족하고 인덱스 성능도 가장 좋은 TSID를 선택했다.
...
UUIDv7은 UUID 표준을 따르기 때문에 128비트(16바이트)를 사용한다. DB에서 BINARY(16)으로 저장하거나 VARCHAR(36)으로 저장하는데, BIGINT(8바이트)와 비교하면 인덱스 크기가 최소 두 배이다.
...
```text
| 48비트 타임스탬프 | 12비트 노드 ID | 14비트 시퀀스 |
```
- **타임스탬프**: 밀리초 단위의 Unix 타임스탬프로, 시간 순 정렬을 보장한다.
- **노드 ID**: 분산 환경에서 각 서버를 구별하는 식별자로, 전역 고유성을 보장한다.
- **시퀀스**: 같은 밀리초 내에서 여러 ID가 생성될 때 순서를 보장한다.
...
TSID를 도입하고 프론트엔드에서 백엔드 로직을 테스트하던 도중에 또 다른 문제가 발생했다. 채널을 생성하고 메시지를 생성했는데, 프론트 화면에 나타나지 않는 것이었다. 
...
이후 프론트 화면에서 해당 채널의 메시지 목록을 모두 확인할 수 있었다.
...
따라서 모든 엔티티가 `BaseEntity`를 상속받아 전역 고유성과 시간 순 정렬, 일관성, Persistable 인터페이스 구현을 통한 성능 최적화의 이점을 얻을 수 있었다.
...
일반적으로 커서 기반 페이징에서 시간 순 정렬을 보장하려면 `(created_at, id)` 복합 인덱스가 필요하다. `created_at`만으로는 동일한 시간에 생성된 레코드의 순서를 보장할 수 없기 때문이다.
...
TSID 도입은 단순히 PK 전략을 바꾸는 것이 아니었다. 애플리케이션 레벨에서 ID를 생성하게 되면서 JPA의 내부 동작에 대한 이해가 필요했고, 백엔드와 프론트엔드 간의 타입 시스템 차이도 직접 체감할 수 있었다.

새로운 기술을 도입하는 것도 마찬가지로 단순히 그 기술을 이해하고 도입하는 것만으로 끝나지 않을 것이다. 이번에 겪은 `SimpleJpaRepository`의 `save()` 문제와 JavaScript `number` 정밀도 문제를 통해 새로운 기술이 기존의 환경에 어떤 영향을 끼칠 지 파악하는 것의 중요성을 몸소 느낄 수 있었다.