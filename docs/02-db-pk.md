...
UUID의 성능 문제를 개선하기 위해 등장한 버전이다. 앞부분에 타임스탬프를 포함하여 시간순 정렬이 가능하기 때문에 UUIDv4에 비해 페이지 분할 문제를 크게 줄일 수 있다. 하지만 여전히 36바이트(바이너리로 저장 시 16바이트)로, 8바이트인 BIGINT에 비해 인덱스 크기가 최소 2배이다.

### 1.3. TSID 선택

PK 후보를 검토하던 도중 TSID(Time-Sorted Unique Identifier)에 대해 알게 되었다.

TSID는 8바이트 BIGINT와 완벽하게 호환되면서도 시간순 정렬과 전역 고유성을 모두 만족한다.
...
따라서 요구사항을 모두 만족하고 인덱스 성능도 가장 좋은 TSID를 선택했다.
...
새로운 기술을 도입하는 것도 마찬가지로 단순히 그 기술을 이해하고 도입하는 것만으로 끝나지 않을 것이다. 이번에 겪은 `SimpleJpaRepository`의 `save()` 문제와 JavaScript `number` 정밀도 문제를 통해 새로운 기술이 기존의 환경에 어떤 영향을 끼칠지 파악하는 것의 중요성을 몸소 느낄 수 있었다.