...
인덱스는 조회(Read) 성능을 끌어올리는 대신 메모리를 추가로 사용하고 쓰기(Write) 성능을 희생하는 트레이드오프 관계를 가진다. 하지만 메신저 서비스는 도메인 특성상 조회보다 쓰기 작업이 더 빈번하므로, 비대한 인덱스를 만드는 것은 쓰기 성능을 저하시키는 요인이 되기 때문에 최대한 지양해야 한다.
