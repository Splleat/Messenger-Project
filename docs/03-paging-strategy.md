...
커서 기반 페이징에서 시간순 정렬을 보장하려면 커서 기준이 되는 컬럼이 고유해야 한다. `created_at`만 사용하면 정확히 `2026-01-01 00:00:00.123`에 3개의 메시지가 동시에 생성되었을 때, 어떤 메시지를 기준으로 해야 할 지 알 수가 없다.

일반적인 해결 방법은 `(created_at, id)` 복합 인덱스를 사용하는 것이다. 하지만 TSID는 타임스탬프와 시퀀스가 ID에 포함되어 있기 때문에 ID 자체가 고유한 시간순 정렬 커서가 될 수 있다.
...
물론 실제 서비스에서는 특정 채널에 속한 메시지만을 필터링해야 하므로 `channelId` 조건이 필수로 포함된다. 만약 UUIDv4처럼 순서나 시간으로 정렬되지 않는 PK를 사용한다면 `(channel_id, created_at, id)`와 같은 무겁고 복잡한 삼중 복합 인덱스를 명시적으로 생성해야 한다.

인덱스는 조회(Read) 성능을 끌어올리는 대신 메모리를 추가로 사용하고 쓰기(Write) 성능을 희생하는 트레이드 오프 관계를 가진다. 하지만 메신저 서비스는 도메인 특성상 조회보다 쓰기 작업이 더 빈번하므로, 비대한 인덱스를 만드는 것은 쓰기 성능을 저하시키는 요인이 되기 때문에 최대한 지양해야 한다.

TSID를 사용하면 `(channel_id, id)` 형태의 직관적인 복합 인덱스 하나만으로 정렬과 필터링 조건을 모두 만족할 수 있다. Auto Increment를 PK로 사용하면 어떨까? Auto Increment 역시 순서를 보장하기 때문에 마찬가지로 `(channel_id, id)` 복합 인덱스 구성이 가능하다. 하지만, 이전 문서에서 언급한 보안 문제와 분산 DB 환경에서의 한계로 인해 고려 대상에서 제외되었다.

결과적으로 TSID를 도입함으로써 인덱스 크기를 줄여 쓰기 성능을 최대한 끌어올리면서 동시에 분산 DB 환경의 확장성까지 모두 고려하는 최적의 설계를 할 수 있었다.
...
하지만 이번 설계의 핵심은 단순히 오프셋을 커서로 바꾼 것만이 아니다. 메신저 서비스의 특성상 사용자는 과거 메시지를 탐색하기도 하고, 읽던 위치에서 다시 대화를 이어가기도 한다. 따라서 단방향 무한 스크롤이 아닌 이전 조회, 다음 조회, 마지막 읽은 위치 기준 조회를 모두 지원하는 양방향 커서 페이징 구조가 필요했다.
