...
class SpaceChannelEnterUseCaseTest {
    ...
    void execute_WhenLastReadMessageIdIsNull_FindsByNewest() {
        ...
        given(channelUserSettingService.registerIfAbsent(userId, channelId))
                .willReturn(setting);
        ...
        // when
        ChannelEnterResult actual = spaceChannelEnterUseCase.execute(userId, spaceId, channelId);

        // then
        assertThat(actual).isEqualTo(result);
        then(spaceMemberService).should().validateParticipant(userId, spaceId);
        then(channelService).should().validateInSpace(spaceId, channelId);
        then(setting).should().updateLastReadMessage(nextCursorId);
    }
    ...
    void execute_WhenLastReadMessageIdIsPresent_FindsByAroundId() {
        ...
        then(channelService)
                .should()
                .validateInSpace(spaceId, channelId);
        then(setting)
                .should()
